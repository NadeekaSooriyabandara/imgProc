...
my_image = imresize(imcrop(Z, [400 130 180 90]), 1);
%[E, Mx, My] = GSobel(my_image, 5);BB = binarizeImage(E);figure;imshow(my_image);figure; imshow(E);figure;imshow(BB);
[I, hor_grad, ver_grad] = getSobel(my_image);

%[hor_grad, ver_grad, GRADIENT] = getGradients(my_image, 1);
non_max = getNonMax(my_image, I, hor_grad, ver_grad, 1);
SS = imgradient(my_image);
post_hysteresis = getHysteresis(my_image, non_max, 1, 0.01, 0.03);
figure; imshow(Z);figure; imshow(my_image);figure; imshow(hor_grad); figure;imshow(ver_grad); figure;imshow(I); figure;imshow(non_max); figure;imshow(post_hysteresis);
